...
async function main() {
    ...
    console.log("\nDeploying PassportVerifier (modified to accept any timestamp)...");
    const PassportVerifier = await hre.ethers.getContractFactory("PassportVerifier");
    const verifier = await PassportVerifier.deploy(SP1_VERIFIER_SEPOLIA, vkey);

    await verifier.waitForDeployment();
    const verifierAddress = await verifier.getAddress();
    console.log("PassportVerifier deployed to:", verifierAddress);
